...
//		if (("`nostd'"=="") | (`cons')) & ("`nopreserve'"=="") & (`stdflyflag'==0) {
		...
		mata: st_local("wvar",`structname'.Wname)
		mata: st_local("touse",`structname'.tousename)
		mata: st_local("toest",`structname'.toestname)
		mata: st_local("holdout",`structname'.holdoutname)
	}
	...
	*** StandardizeData() standardizes the estimation sample.
	...
	if "`setupstruct'"~="" {
		...
		mata: st_numscalar("e(N)",`setupstruct'.total_trials)
		...
		*** lambda
		...
		if ("`rigorous'"!="") & ("`lambda'"=="") {
			...
			if `lambdanflag'==0 {
				mata: `lambdas' = `lambdas' * 1/`data'.total_trials
			}
		}
		else {
